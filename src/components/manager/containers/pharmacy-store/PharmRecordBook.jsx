...
const PharmRecordBook = () => {
    ...
    const getRecordDetails = async () => {
        ...
        let nextPageURL = `${root}/dept/pharm-log?lastCreatedAt=${pageParams.lastCreatedAt}&lastId=${pageParams.lastId}&limit=${pageParams.limit}&sortBy=${pageParams.sortBy}&sortOrder=${pageParams.sortOrder}`
                ...
                <Flex className='my-6' justify={"end"}>
                    <Button
                        ...
                        onClick={() => {
                            ...
                            navigate(
                                `/admin/pharmacy-store/record-book?lastCreatedAt=${parseUrlParams(details.pagination.nextPage)["lastCreatedAt"]}&lastId=${parseUrlParams(details.pagination.nextPage)["lastId"]}&limit=${parseUrlParams(details.pagination.nextPage)["limit"]}&sortBy=${parseUrlParams(details.pagination.nextPage)["sortBy"]}&sortOrder=${parseUrlParams(details.pagination.nextPage)["sortOrder"]}`
                            )
