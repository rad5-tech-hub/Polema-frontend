...
const EditRole = () => {
  ...
  const getARole = async () => {
    ...
    try {
      ...
      const permissionIds = navParents
        ...
        .map((permission) => String(permission.id)); // Convert IDs to strings for consistency

      setDefaultRoles(new Set(permissionIds)); // Use Set for efficient lookups
      setSelectedCheckboxes(permissionIds); // Sync selectedCheckboxes with defaultRoles
    } catch (error) {
  ...
  const handleCheckboxChange = (event) => {
    ...
    setDefaultRoles((prevRoles) => {
      ...
      // Sync selectedCheckboxes for form submission
      setSelectedCheckboxes(Array.from(updatedRoles));
      return updatedRoles; // Return the updated Set
    });
  };
                  ...
                  {permissionBox.map((box, index) => (
                    <Card className="w-full" key={index}>
                      <Flex justify={"between"} align={"center"}>
                        <Text className="font-medium">{box.navParentName}</Text>
                      </Flex>
