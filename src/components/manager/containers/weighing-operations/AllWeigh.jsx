...
//           }
//           // Only append new URLs, don't overwrite unless starting fresh
//           return pageUrl ? [...prev, ...newUrls] : newUrls.length > 0 ? newUrls : prev;
//         });
//       } else {
//         const endpoint =
//           filter === "completed" ? "/customer/view-weighs" : "/customer/view-saved";
...
//             return pageUrl ? [...prev, newUrl] : [newUrl];
//           }
//           return prev;
//         });
//       }
...
//       fetchWeighDetails(null, null, paginationUrls[nextIndex] || paginationUrls[currentPageIndex]);
//     }
//   };
...
//       fetchWeighDetails(null, null, prevIndex === 0 ? null : paginationUrls[prevIndex]);
//     }
//   };
...
//                       {selectedWeigh?.authToWeigh?.status || "Pending"}
//                     </Text>
//                   </div>
//                 </div>
//               </>
...
//                       {item?.authToWeigh?.status ? (
//                         <FontAwesomeIcon
//                           icon={faSquare}
//                           className={`${checkStatus(item?.authToWeigh?.status)}`}
//                         />
//                       ) : (
//                         <FontAwesomeIcon
//                           icon={faSquare}
...
//                         {_.upperFirst(item?.authToWeigh?.status) || "Pending"}
//                       </span>
//                     </div>
  ...
  const handleNextPage = () => {
    if (currentPageIndex < paginationUrls.length) {
      ...
      fetchWeighDetails(
        null,
        null,
        paginationUrls[nextIndex] || paginationUrls[currentPageIndex]
      );
    }
  };
  ...
  const handlePrevPage = () => {
    if (currentPageIndex > 0) {
      ...
      fetchWeighDetails(
        null,
        null,
        prevIndex === 0 ? null : paginationUrls[prevIndex]
      );
    }
  };
                ...
                <Table.Cell>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        {item?.authToWeigh?.status ? "Completed" : "Pending"}
                      </span>
                    </div>
                    {!item?.authToWeigh?.status && (
                      <DropdownMenu.Root>
