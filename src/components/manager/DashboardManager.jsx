...
const DashBoardManager = ({ children }) => {
  ...
  const [openDropdownIndex, setOpenDropdownIndex] = useState(null); // Tracks the currently open dropdown
  ...
  const fetchNav = async () => {
    ...
    try {
      ...
      console.log(error);
      setLoading(false);
      toast.error(error?.response?.message?.data || "An error occurred.");
    }
  };
